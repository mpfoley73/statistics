...
The posterior predictive distribution of $\tilde{d}$ is

$$
f(\tilde{d} | x) = \int f(\tilde{d}|\lambda) f(\lambda | \textbf{D}) d\lambda
$$

Our sandwich example has a well defined functional solution: the expected value from $\mathrm{Gamma}(1104, 16.2)$, which is $1104/16.2 = 68$. Had we not known this, we could have simulated posterior values (Monte Carlo simulation) and taken the mean and variance. The procedure is to take a random sample of, say, 1,000 $\lambda$ values from the gamma posterior distribution, then for each $\lambda$ draw a single random $\tilde{d}$ from the Poisson distribution. 
...
So on any given day, the predicted value of sandwich sales is `r round(mean(d_tilde), 1)` with 95% prediction interval `r quantile(d_tilde, c(.025, .975)) %>% comma(.1) %>% paste(collapse = ", ")`. The probability of exceeding 80 sandwiches, $P(\tilde{d} > 80 | \textbf{D})$, is simply `mean(d_tilde > 80)` = `r mean(d_tilde > 80) %>% percent(.1)`. And 99% of the time, sandwich sales will be less than `quantile(d_tilde, .99)` = `r quantile(d_tilde, .99)`.
...
The posterior probability that average sales on Mon are less than Tue is only `r percent(mean(lambda_r_mon < lambda_r_tue), 1)`.

You can also estimate which day of the week has the highest sandwich sales.
...
In Bayesian statistics, the normal distribution is parameterized with the precision, the inverse variance, $\tau = 1 / \sigma^2$: $X = N(\mu, \tau)$
