...
H0_str <- "H0: p = .50"
H0 <- .50
act_str_lwr <- "Actual: pi = .40"
pop_lwr <- .40
act_str_upr <- "Actual: pi = .60"
pop_upr <- .60
pal <- c("gray30", "darkgoldenrod", "darkgoldenrod")
names(pal) <- c(H0_str, act_str_lwr, act_str_upr)

sampling_dist <- 
  tibble(
    grp = factor(c(rep(H0_str, n), rep(act_str_lwr, n), rep(act_str_upr, n)), 
                 levels = c(H0_str, act_str_lwr, act_str_upr)),
    cnt = rep(seq(1, n, 1), 3),
...
sampling_dist |>
  ...
  geom_area(data = sampling_dist |> filter(grp == H0_str & cnt >= crit_val_upr),
            aes(x = prop, y = dens, fill = grp),
            alpha = .2) +
  geom_area(data = sampling_dist |> filter(grp == act_str_lwr & cnt >= crit_val_lwr),
            aes(x = prop, y = dens, fill = grp),
            alpha = .2) +
  geom_area(data = sampling_dist |> filter(grp == act_str_upr & cnt <= crit_val_upr),
            aes(x = prop, y = dens, fill = grp),
            alpha = .2) +
  annotate("text", x = c(.45, .55), y = .005, label = "beta", parse = TRUE, size = 4.5, 
           color = pal[act_str_lwr]) +
  annotate("text", x = c(.42, .58), y = .0025, label = "alpha / 2", parse = TRUE, size = 3.0, 
           color = pal[H0_str]) +
  geom_vline(xintercept = H0, color = pal[H0_str], linetype = 2, linewidth = 1) +
  geom_vline(xintercept = c(pop_lwr, pop_upr), color = pal[act_str_lwr], 
...
~20% of the area under *`r act_str_lwr`* is to the right of `r comma(crit_prop_lwr, .01)` and ~20% of the area under *`r act_str_upr`* is to the left of `r comma(crit_prop_upr, .01)`.
...
sampling_dist <- 
  tibble(
    grp = factor(c(rep(H0_str, 1001), rep(act_str, 1001)), levels = c(H0_str, act_str)),
    prop_diff = rep(seq(-.2, .3, .0005), 2),
    dist_mean = c(rep(H0, 1001), rep(pop, 1001)),
    dens = pmap_dbl(list(prop_diff, dist_mean), \(x, M) dnorm(x, mean = M, sd = SE))
  )

sampling_dist |>
  ggplot() +
  geom_line(aes(x = prop_diff, y = dens, color = grp, fill = grp)) +
  geom_area(aes(x = prop_diff, fill = grp, 
                y = if_else(prop_diff >= crit_val & grp == H0_str, dens, NA_real_)), 
            alpha = .2) +
  geom_area(aes(x = prop_diff, fill = grp, 
                y = if_else(prop_diff <= crit_val & grp == act_str, dens, NA_real_)), 
            alpha = .2) +
  annotate("text", x = .03, y = .5, label = "beta", parse = TRUE, size = 4.5, 
           color = pal[act_str]) +
  annotate("text", x = .08, y = .5, label = "alpha", parse = TRUE, size = 4.5, 
           color = pal[H0_str]) +
  geom_vline(xintercept = H0, color = pal[H0_str], linetype = 2, linewidth = 1) +
  geom_vline(xintercept = pop, color = pal[act_str], linetype = 2, linewidth = 1) +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  scale_x_continuous(breaks = c(H0, pop, crit_val), labels = comma_format(.01)) +
  ...
  labs(
    ...
    subtitle = str_wrap(glue(
      "Effect size h = .2 (10 pct pts), 80% power, Sample size n = {comma(n)} ",
      "for each group. Differences >= {comma(crit_val, .01)} reject {H0_str} at alpha = .05."
    ), 95)
  )
```
...
H0_str <- "H0: d = 0"
H0 <- 0
act_str_lwr <- "Actual: D = -.1"
pop_lwr <- -.10
act_str_upr <- "Actual: D = .1"
pop_upr <- +.10
pal <- c("gray30", "darkgoldenrod", "darkgoldenrod")
names(pal) <- c(H0_str, act_str_lwr, act_str_upr)
...
sampling_dist <- 
  tibble(
    grp = factor(c(rep(act_str_lwr, 1001), rep(H0_str, 1001), rep(act_str_upr, 1001)),
                 levels = c(act_str_lwr, H0_str, act_str_upr)),
    prop_diff = rep(seq(-.25, .25, .0005), 3),
    diff_mean = c(rep(pop_lwr, 1001), rep(H0, 1001), rep(pop_upr, 1001)),
    dens = pmap_dbl(list(prop_diff, diff_mean), \(x, M) dnorm(x, mean = M, sd = SE)),
  )

sampling_dist |>
  ggplot() +
  geom_line(aes(x = prop_diff, y = dens, color = grp)) +
  geom_area(aes(x = prop_diff, fill = grp, 
                y = if_else(prop_diff >= crit_val_upr & grp == H0_str, dens, NA_real_)), 
            alpha = .2) +
  geom_area(aes(x = prop_diff, fill = grp, 
                y = if_else(prop_diff <= crit_val_lwr & grp == H0_str, dens, NA_real_)), 
            alpha = .2) +
  geom_area(aes(x = prop_diff, fill = grp, 
                y = if_else(prop_diff <= crit_val_upr & grp == act_str_upr, dens, NA_real_)),
            alpha = .2) +
  geom_area(aes(x = prop_diff, fill = grp, 
                y = if_else(prop_diff >= crit_val_lwr & grp == act_str_lwr, dens, NA_real_)), 
            alpha = .2) +
  annotate("text", x = .04, y = .5, label = "beta", parse = TRUE, size = 4.5, 
           color = pal[act_str_lwr]) +
  annotate("text", x = .08, y = .5, label = "alpha", parse = TRUE, size = 4.5, 
           color = pal[H0_str]) +
  annotate("text", x = -.04, y = .5, label = "beta", parse = TRUE, size = 4.5, 
           color = pal[act_str_upr]) +
  annotate("text", x = -.08, y = .5, label = "alpha", parse = TRUE, size = 4.5, 
           color = pal[H0_str]) +
  geom_vline(xintercept = H0, color = pal[H0_str], linetype = 2, linewidth = 1) +
  geom_vline(xintercept = pop_lwr, color = pal[act_str_lwr], linetype = 2, linewidth = 1) +
  geom_vline(xintercept = pop_upr, color = pal[act_str_upr], linetype = 2, linewidth = 1) +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  scale_x_continuous(breaks = c(H0, pop_lwr, pop_upr, crit_val_lwr, crit_val_upr), 
                     labels = comma_format(.01)) +
  ...
  labs(
    ...
    subtitle = str_wrap(glue(
      "Effect size h = .2 (10 pct pts), 80% power, Sample size n = {comma(n)} ",
      "for each group. Differences<= {comma(crit_val_lwr, .01)} or ",
      ">= {comma(crit_val_upr, .01)} reject H0 at alpha = .05."
    ), 100)
  )
```
...
sampling_dist <- 
  tibble(
    grp = factor(c(rep(H0_str, 201), rep(act_str, 201)), 
                 levels = c(H0_str, act_str)),
    rating = rep(seq(3, 5, .01), 2),
    x = (rating - if_else(grp == H0_str, H0, pop)) / se,
    dens = map_dbl(x, \(x) dt(x, df = n - x))
  )

sampling_dist |>
  ggplot() +
  geom_line(aes(x = rating, y = dens, color = grp)) +
  geom_area(aes(x = rating, fill = grp, 
                y = if_else(rating >= crit_rating & grp == H0_str, dens, NA_real_)), 
            alpha = .2) +
  geom_area(aes(x = rating, color = grp, fill = grp, 
                y = if_else(rating <= crit_rating & grp == act_str, dens, NA_real_)), 
            alpha = .2) +
  annotate("text", x = 4.2, y = .03, label = "beta", parse = TRUE, size = 4.5, 
           color = pal[act_str]) +
  annotate("text", x = 4.4, y = .03, label = "alpha", parse = TRUE, size = 4.5, 
           color = pal[H0_str]) +
  geom_vline(xintercept = H0, color = pal[H0_str], linetype = 2, linewidth = 1) +
  geom_vline(xintercept = pop, color = pal[act_str], linetype = 2, linewidth = 1) +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  scale_x_continuous(breaks = c(3, pop, H0, 5, crit_rating), labels = comma_format(.1)) +
  ...
  labs(
    ...
    subtitle = str_wrap(glue(
      ...
      "Average ratings > {comma(crit_rating, .1)} reject {H0_str} at alpha = .05."
    ), 95)
  )
```
...
H0_str <- "H0: mean = 4"
H0 <- 4
act_str_lwr <- "Actual: mu = 3.5"
act_lwr <- 3.5
act_str_upr <- "Actual: mu = 4.5"
act_upr <- 4.5
pal <- c("darkgoldenrod", "gray30", "darkgoldenrod")
names(pal) <- c(act_str_lwr, H0_str, act_str_upr)

sampling_dist <- 
  tibble(
    grp = factor(c(rep(H0_str, 201), rep(act_str_lwr, 201), rep(act_str_upr, 201)), 
                 levels = c(act_str_lwr, H0_str, act_str_upr)),
    rating = rep(seq(3, 5, .01), 3),
    x = (rating - case_when(grp == H0_str ~ H0,
                            grp == act_str_lwr ~ act_lwr, 
                            TRUE ~ act_upr)) / se,
    dens = map_dbl(x, \(x) dt(x, df = n - x))
  )
...
sampling_dist |>
  ...
  geom_line(aes(x = rating, y = dens, color = grp)) +
  geom_area(aes(x = rating, fill = grp, 
                y = if_else(rating <= crit_rating_lwr & grp == H0_str, dens, NA_real_)), 
            alpha = .2) +
  geom_area(aes(x = rating, fill = grp, 
                y = if_else(rating >= crit_rating_lwr & grp == act_str_lwr, dens, NA_real_)),
            alpha = .2) +
  geom_area(aes(x = rating, fill = grp, 
                y = if_else(rating >= crit_rating_upr & grp == H0_str, dens, NA_real_)), 
            alpha = .2) +
  geom_area(aes(x = rating, fill = grp, 
                y = if_else(rating <= crit_rating_upr & grp == act_str_upr, dens, NA_real_)),
            alpha = .2) +
  annotate("text", x = 3.8, y = .03, label = "beta", parse = TRUE, size = 4.5, 
           color = pal[act_str_lwr]) +
  annotate("text", x = 3.65, y = .03, label = "alpha", parse = TRUE, size = 4.5, 
           color = pal[H0_str]) +
  annotate("text", x = 4.2, y = .03, label = "beta", parse = TRUE, size = 4.5, 
           color = pal[act_str_upr]) +
  annotate("text", x = 4.35, y = .03, label = "alpha", parse = TRUE, size = 4.5, 
           color = pal[H0_str]) +
  geom_vline(xintercept = act_lwr, color = pal[act_str_lwr], linetype = 2, linewidth = 1) +
  geom_vline(xintercept = H0, color = pal[H0], linetype = 2, linewidth = 1) +
  geom_vline(xintercept = act_upr, color = pal[act_str_upr], linetype = 2, linewidth = 1) +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  scale_x_continuous(breaks = c(3, act_lwr, 4, 4.5, 5, crit_rating_lwr, crit_rating_upr), labels = comma_format(.1)) +
  coord_cartesian(xlim = c(3, 5)) +
  guides(fill = "none") +
  theme(axis.text.x = element_text(angle = 0, vjust = .5)) +
  labs(
    ...
    subtitle = str_wrap(glue(
      "Effect size d = .5, 80% power, Sample size n = {comma(n)}. ",
      "Average ratings between {comma(crit_rating_lwr, .1)} and ",
      "{comma(crit_rating_upr, .1)} reject H0 at alpha = .05."
    ), 95)
  )
```
...
sampling_dist <- 
  tibble(
    ...
    x = (rating_diff - if_else(grp == H0_str, H0, act)) / SE,
    dens = map_dbl(x, \(x) dt(x, df = df))
  )

sampling_dist |>
  ggplot() +
  geom_line(aes(x = rating_diff, y = dens, color = grp)) +
  geom_area(aes(x = rating_diff, fill = grp, 
                y = if_else(rating_diff >= crit_diff & grp == H0_str, dens, NA_real_)), 
            alpha = .2) +
  geom_area(aes(x = rating_diff, fill = grp, 
                y = if_else(rating_diff <= crit_diff & grp == act_str, dens, NA_real_)),
            alpha = .2) +
  annotate("text", x = .25, y = .03, label = "beta", parse = TRUE, size = 4.5, 
           color = pal[act_str]) +
  annotate("text", x = .40, y = .03, label = "alpha", parse = TRUE, size = 4.5, 
           color = pal[H0_str]) +
  geom_vline(xintercept = H0, color = pal[H0_str], linetype = 2, linewidth = 1) +
  geom_vline(xintercept = act, color = pal[act_str], linetype = 2, linewidth = 1) +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  scale_x_continuous(breaks = c(-1, -.5, H0, act, 1, crit_diff), labels = comma_format(.1)) +
  ...
  labs(
    ...
    subtitle = str_wrap(glue(
      ...
      "n2 = {comma(n2)}. ",
      "Diff in averages > {comma(crit_diff, .1)} reject {H0_str} at alpha = .05."
    ), 95)
  )
```
...
H0_str <- "H0: d = 0"
H0 <- 0
act_str_lwr <- "Actual: D = -0.5"
act_lwr <- -0.5
act_str_upr <- "Actual: D = +0.5"
act_upr <- 0.5
pal <- c("darkgoldenrod", "gray30", "darkgoldenrod")
names(pal) <- c(act_str_lwr, H0_str, act_str_upr)
...
sampling_dist <- 
  tibble(
    grp = factor(c(rep(act_str_lwr, 201), rep(H0_str, 201), rep(act_str_upr, 201)), 
                 levels = c(act_str_lwr, H0_str, act_str_upr)),
    rating_diff = rep(seq(-1, 1, .01), 3),
    x = (rating_diff - case_when(grp == H0_str ~ 0, 
                                 grp == act_str_lwr ~ -0.5,
                                 grp == act_str_upr ~ 0.5)) / SE,
    dens = map_dbl(x, \(x) dt(x, df = df))
  )
...
sampling_dist |>
  ...
  geom_line(aes(x = rating_diff, y = dens, color = grp)) +
  geom_area(aes(x = rating_diff, fill = grp, 
                y = if_else(rating_diff <= crit_diff_l & grp == H0_str, dens, NA_real_)), 
            alpha = .2) +
  geom_area(aes(x = rating_diff, fill = grp, 
                y = if_else(rating_diff >= crit_diff_u & grp == H0_str, dens, NA_real_)), 
            alpha = .2) +
  geom_area(aes(x = rating_diff, fill = grp, 
                y = if_else(rating_diff >= crit_diff_l & grp == act_str_lwr, dens, NA_real_)),
            alpha = .2) +
  geom_area(aes(x = rating_diff, color = grp, fill = grp, 
                y = if_else(rating_diff <= crit_diff_u & grp == act_str_upr, dens, NA_real_)),
            alpha = .2) +
  annotate("text", x = .25, y = .03, label = "beta", parse = TRUE, size = 4.5, 
           color = pal[act_str_lwr]) +
  annotate("text", x = .35, y = .03, label = "alpha", parse = TRUE, size = 4.5, 
           color = pal[H0_str]) +
  annotate("text", x = -.25, y = .03, label = "beta", parse = TRUE, size = 4.5, 
           color = pal[act_str_upr]) +
  annotate("text", x = -.35, y = .03, label = "alpha", parse = TRUE, size = 4.5, 
  ...
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  scale_x_continuous(breaks = c(-1, -.5, 0, .5, 1, crit_diff_l, crit_diff_u), 
                     labels = comma_format(.1)) +
  coord_cartesian(xlim = c(-1, 1)) +
  guides(fill = "none") +
  theme(axis.text.x = element_text(angle = 0, vjust = .5)) +
