...
data.frame(theta, prior, posterior) %>% 
  ...
  geom_line(aes(x = theta, y = pi, color = name), linewidth = 1) +
  scale_x_continuous(breaks = theta) +
  theme_minimal() +
  theme(panel.grid.minor = element_blank()) +
...
```{r}
D100 <- Rlab::rbern(100, p = 0.7) %>% as.numeric()
...
The problem here is that you never know $\mu$ or $\tau$, so you cannot use the posterior formulas directly. Instead, you need to use sampling. In particular, you use the *Gibbs sampler*. You set $\mu$ and $\tau$ to some initial values and use the posterior equations to estimate new values for $\mu$ and $\tau$, and repeat. This is called Markov chain Monte Carlo simulation because you are chaining the simulations. The method of sampling from a conditional posterior is called the Gibbs sampler.

Let's implement a Gibbs sampler with a data set of 100 random values from the normal distribution. Use vague priors of $\mu \sim N(\mu_0 = 0, \tau_0 = 1/100^2)$ and $\tau = \sim \text{Gamma}(a = .01, b = .01)$. Start by assigning starting values, $\mu*$ and $\tau*$). Given $\tau = \tau*$, sample a new value of $\mu*$ from the normal distribution. Given $\mu = \mu*$, sample a new value of $\tau*$ from the gamma distribution. Then repeat.
...
# vague priors: mu ~ N(0, 1/100^2); tau ~ Gamma(.01, 01)
mu_0 <- 0
tau_0 <- 1 / 100^2
a <- .01
b <- .01
...
# mu|tau,y 
sample_mu <- function(tau) {
  mean_mu <- (n * mean(y) * tau + mu_0 * tau_0) / (n * tau + tau_0)
  precision_mu <- n * tau + tau_0
  rnorm(1, mean_mu, 1 / sqrt(precision_mu))
}

# tau|mu,y
sample_tau <- function(mu) {
  rgamma(1, a + n/2, b + .5 * sum((y - mu)^2))
}
...
# Gibbs sampler
for(i in 2:n_iter){
  mu_sample[i] <- map_dbl(tau_sample[i-1], sample_mu)
  tau_sample[i] <- map_dbl(mu_sample[i-1], sample_tau)
}

# posterior mu
mean(mu_sample); quantile(mu_sample, c(.025, .975))

# posterior tau
mean(tau_sample); quantile(tau_sample, c(.025, .975))

tibble(iter = 1:n_iter, mu = mu_sample, tau = tau_sample) %>%
  pivot_longer(cols = c(mu, tau)) %>% 
  ggplot(aes(x = value)) + geom_density() + 
  facet_wrap(facets = vars(name), scales = "free") +
  labs(title = "Posterior Distributions", x = NULL)
```
...
Let's apply this using anthropological data collected by Nancy Howell.
